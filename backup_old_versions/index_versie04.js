...
app.get("/metar.geojson", async (req, res) => {
  try {
    ...
    // 1️⃣ Probeer nieuwste files op te halen
    let latestFiles = {};
    try {
      latestFiles = await fetchLatestFilenamesForStations(stations);
    } catch (e) {
      console.warn("KNMI filelijst niet bereikbaar, gebruik alleen cache");
    }
    ...
    for (const station of stations) {
      ...
      // 2️⃣ Als we een nieuw bestand kennen → proberen te downloaden
      if (latestFiles[station]) {
        xml = await updateMetarForStation(station, latestFiles[station]);
      }

      // 3️⃣ Fallback: altijd lokale cache
      if (!xml) {
        xml = loadMetarXml(station);
      }

      // 4️⃣ Als we écht niets hebben → overslaan
      if (!xml) continue;
...
/* pp.get("/metar.geojson", async (req, res) => {
  try {
    const stations = Object.keys(STATIONS);
    const latest = await fetchLatestFilenamesForStations(stations);

    const features = [];

    for (const station of stations) {
      const filename = latest[station];
      if (!filename) continue;

      const xml = await downloadXmlForFilename(filename);
      const props = parseIwxxmMetar(xml);

      // Als station niet gevonden kan worden in XML, val terug op de key die we opvroegen
      const stationCode = props.station || station;

      const coords = STATIONS[station];
      features.push({
        type: "Feature",
        geometry: { type: "Point", coordinates: [coords.lon, coords.lat] },
        properties: { ...props, station: stationCode, filename }
      });
    }
